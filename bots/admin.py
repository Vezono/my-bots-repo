    ...
    if is_admin(m.chat.id, m.from_user.id) and not is_admin(m.chat.id, m.reply_to_message.from_user.id):
        ...
        try:
            ...
            if m.reply_to_message:
                ahref = '[' + m.reply_to_message.from_user.first_name + ']' + '(tg://user?id=' + str(
...
def unmutee(m):
    ...
    if is_admin(m.chat.id, m.from_user.id):
        ahref = '[' + m.reply_to_message.from_user.first_name + ']' + '(tg://user?id=' + str(
            m.reply_to_message.from_user.id) + ')'
        bot.restrict_chat_member(can_send_messages=True, can_send_other_messages=True,
                                 user_id=m.reply_to_message.from_user.id, chat_id=m.chat.id)
...
def ping(m):
    ...
    if m.reply_to_message:
        tts = tts.format(str(m.chat.id), m.chat.type, m.reply_to_message.from_user.first_name,
