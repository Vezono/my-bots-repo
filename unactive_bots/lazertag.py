...
def start(m):
    ...
    if x == None:
        ...
        bot.send_message(m.chat.id,
                         m.from_user.first_name + ', приветствую в игре "Лазертаг"! Цель этой игры - остаться единственной ' +
...
def preparegame(m):
    ...
    if no == 0:
        ...
        msg = bot.send_message(m.chat.id, 'Начинаем сбор игроков! Жмите /tagjoin для вступления в игру.',
                               reply_markup=kb)
    ...
    if 'fight' in call.data:
        ...
        if player != None:
            if 'shoot' in call.data:
                for ids in game['teams']:
                    if ids['id'] != player['team']:
                        for idss in ids['players']:
                            if idss['dead'] != 1:
                                kb.add(types.InlineKeyboardButton(text='💢' + idss['name'],
                                                                  callback_data='fight target ' + chat + ' ' + str(
                                                                      idss['id'])))
            ...
            if 'def' in call.data:
                ...
                b2 = []
                for ids in s:
                    b.append(types.InlineKeyboardButton(text='🔵+' + str(ids) + '%',
                                                        callback_data='fight def1 ' + chat + ' ' + str(ids)))
                for ids in s2:
                    b2.append(types.InlineKeyboardButton(text='🔵+' + str(ids) + '%',
                                                         callback_data='fight def1 ' + chat + ' ' + str(ids)))
                for ids in m:
                    d.append(types.InlineKeyboardButton(text='🔵' + str(ids) + '%',
                                                        callback_data='fight def1 ' + chat + ' ' + str(ids)))
                for ids in m2:
                    d2.append(types.InlineKeyboardButton(text='🔵' + str(ids) + '%',
                                                         callback_data='fight def1 ' + chat + ' ' + str(ids)))
                kb.add(b[0], b[1], b[2])
                ...
                medit('Выберите мощность щита. Текущая мощность: ' + str(player['currentdef']) + '%',
                      player['message'].chat.id, player['message'].message_id, reply_markup=kb)
            ...
            if 'target' in call.data:
                ...
                for ids in s:
                    b.append(types.InlineKeyboardButton(text='🔴+' + str(ids) + '%',
                                                        callback_data='fight target1 ' + chat + ' ' + str(
                                                            target) + ' ' + str(ids)))
                for ids in s2:
                    b2.append(types.InlineKeyboardButton(text='🔴+' + str(ids) + '%',
                                                         callback_data='fight target1 ' + chat + ' ' + str(
                ...
                medit('Выберите силу заряда. Текущая сила: ' + str(player['currentcharge']) + '%',
                      player['message'].chat.id, player['message'].message_id, reply_markup=kb)
            ...
            if 'fire' in call.data:
                ...
                if enemy != None:
                    ...
                    medit('Вы направляете свой лазер на ' + enemy['name'] + '... Ждите окончания хода.',
                          player['message'].chat.id, player['message'].message_id)
                    player['message'] = None
                    check(game)
                else:
            ...
            if 'reloadgun' in call.data:
                ...
                medit('Вы заряжаете лазер! Ждите окончания хода.', player['message'].chat.id,
        ...
        for ids in game['teams']:
            for idss in ids['players']:
                if idss['action'] == 'attack':
                    if idss['takendmg'] <= 0:
                        game['res'] += '🔴|' + idss['name'] + ' заряжает лазер на ' + str(
                            idss['currentcharge']) + '% и стреляет в ' + idss['target']['name'] + '!\n'
                    else:
                ...
                if idss['action'] == 'def' and 'dead' not in idss['effects']:
                    if idss['currentdef'] < 0:
                        if idss['takendmg'] > 0:
                            ...
                            idss['hp'] -= idss['takendmg']
                            game['res'] += '🔋💔|' + idss['name'] + ' зарядил ' + str(
                                idss['currentdef'] - (2 * idss['currentdef'])) + '% щита и потерял ' + str(
                                idss['takendmg']) + '% хп!\n'
                        else:
                            game['res'] += '🔋|' + idss['name'] + ' зарядил ' + str(
                                idss['currentdef'] - (2 * idss['currentdef'])) + '% щита!\n'
                    else:
                        if idss['currentdef'] >= idss['takendmg']:
                            l = int(idss['takendmg'] / 2)
                            idss['lazer'] += l
                            idss['shield'] -= idss['currentdef']
                            game['res'] += '🔵|' + idss['name'] + ' блокирует весь входящий урон (' + str(
                                idss['takendmg']) + ')! Потеряно ' + str(idss['currentdef']) + '% заряда щита; ' + \
                            ...
                            idss['takendmg'] -= idss['currentdef']
                            idss['hp'] -= idss['takendmg']
                            game['res'] += '🔵💔|' + idss['name'] + ' тратит ' + str(
                                idss['currentdef']) + '% щита, но блокирует не весь входящий урон! Потеряно ' + str(
                                idss['takendmg']) + '% хп; ' + \
                                           'восстановлено ' + str(l) + '% энергии лазера!\n'
        for ids in game['teams']:
            for idss in ids['players']:
                if idss['action'] == 'reload' and 'dead' not in idss['effects']:
                    if idss['takendmg'] > 0:
                        idss['hp'] -= idss['takendmg']
                        game['res'] += '🔋💔|' + idss[
                            'name'] + ' восстанавливает 25% лазера, но попадает под огонь! Потеряно ' + str(
                            idss['takendmg']) + '% хп.\n'
                    else:
            ...
            for idss in ids['players']:
                if idss['action'] == None and 'dead' not in idss['effects']:
                    if idss['takendmg'] > 0:
                        idss['hp'] -= idss['takendmg']
                        game['res'] += '😴💔|' + idss[
                            'name'] + ' пропускает ход и подставляется под лазер! Потеряно ' + str(
                            idss['takendmg']) + '% хп.\n'
                    else:
...
def startgame(game):
    ...
    for ids in game['teams']:
        for idss in ids['players']:
            sendmenu(idss, game, idss['team'])
    bot.send_message(game['id'],
                     'Начинаем перестрелку! Приготовьте свои лазер-пушки и энергетические щиты и переключайтесь в личку бота!')
...
def sendmenu(player, game, team):
    ...
    for ids in team['players']:
        text += ids['name'] + ':\n' + '♥️:' + str(ids['hp']) + '%, 🔵:' + str(ids['shield']) + '%, 🔴:' + str(
            ids['lazer']) + '%\n\n'
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton(text='🔴Стрельба', callback_data='fight shoot ' + str(game['id'])),
           types.InlineKeyboardButton(text='🔵Защита', callback_data='fight def ' + str(game['id'])))
