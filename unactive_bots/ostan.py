...
def new_chat_members(m):
    if True:
        win_xp.send_photo(chat_id, welcome, caption='Добро пожаловать! Для запуска Биос, позовите его!',
...
def biosmessages(m):
    try:
        if 'биос' in m.text.lower():
            ...
            Keyboard.add(*butts)
            win_7.send_message(m.chat.id, 'Вас приветствует установщик браузера Амиго. Продолжить?',
